...
function looksLikePubkey(buf: Buffer, offset: number) {
  if (offset + 32 > buf.length) return false;
  const slice = buf.slice(offset, offset + 32);
  // not all zeros
  if (slice.every(b => b === 0)) return false;
  // common-sense bound: pubkeys are unlikely to be ASCII letters only
  let asciiCount = 0;
  for (let i = 0; i < slice.length; i++) {
    const c = slice[i];
    if (c >= 32 && c <= 127) asciiCount++;
  }
  // if the slice is entirely printable ascii, it's suspicious (not raw pubkey)
  if (asciiCount === 32) return false;
  // Heuristic passed
  return true;
}

function scanForDiscriminator(buf: Buffer, disc: Buffer, maxOffset = 128) {
  const limit = Math.min(maxOffset, Math.max(0, buf.length - disc.length));
  for (let i = 0; i <= limit; i++) {
    if (buf.slice(i, i + disc.length).equals(disc)) return i;
  }
  return -1;
}

function safeReadUInt8(buf: Buffer, off: number) {
  if (off + 1 > buf.length) throw new Error('out-of-bounds');
  return buf.readUInt8(off);
}

function safeReadBigUInt64LE(buf: Buffer, off: number) {
  if (off + 8 > buf.length) throw new Error('out-of-bounds');
  return buf.readBigUInt64LE(off);
}

function readPubkeyBase58(buf: Buffer, offset: number) {
  const slice = buf.slice(offset, offset + 32);
  return bs58.encode(slice);
}

export function isRecipeAccount(data: Buffer | Uint8Array | null | undefined): boolean {
  if (!data) return false;
  const buf = Buffer.from(data);
  if (buf.length < RECIPE_DISCRIMINATOR.length) return false;
  if (buf.slice(0, RECIPE_DISCRIMINATOR.length).equals(RECIPE_DISCRIMINATOR)) return true;
  ...
  try {
    ...
    if (looksLikePubkey(buf, off) && looksLikePubkey(buf, off + 32) && looksLikePubkey(buf, off + 64)) return true;
  } catch (e) {}
  return false;
}

export function decodeRecipe(data: Buffer | Uint8Array): any | null {
  const buf = Buffer.from(data);
  if (!isRecipeAccount(buf)) return null;
  // Require at least the fixed header size for Recipe (approx 216 bytes) after the discriminator
  if (buf.length < RECIPE_DISCRIMINATOR.length + 216) return null;
  ...
  if (foundAt >= 0) off = foundAt + RECIPE_DISCRIMINATOR.length;
  try {
    const version = buf.readUInt8(off); off += 1;
    const domain = readPubkeyBase58(buf, off); off += 32;
    const category = readPubkeyBase58(buf, off); off += 32;
    const creator = readPubkeyBase58(buf, off); off += 32;
    const duration = Number(buf.readBigInt64LE(off)); off += 8;
    const min_duration = Number(buf.readBigInt64LE(off)); off += 8;
    const namespace = buf.slice(off, off + 32); off += 32;
    const status = buf.readUInt8(off); off += 1;
    const fee_amount = Number(buf.readBigUInt64LE(off)); off += 8;
    // fee_recipient is an Option<NonSystemPubkey> — Carbon seems to encode as 1 byte + 32 bytes when present
    let fee_recipient: string | null = null;
    const maybeFeeFlag = buf.readUInt8(off); off += 1;
    if (maybeFeeFlag === 1) {
      fee_recipient = readPubkeyBase58(buf, off); off += 32;
    }
    const usage_count = Number(buf.readBigUInt64LE(off)); off += 8;
    const usage_limit = Number(buf.readBigUInt64LE(off)); off += 8;
    // Sanity: counts shouldn't be absurd
    if (usage_count < 0 || usage_count > 1_000_000) return null;
    if (usage_limit < 0 || usage_limit > 1_000_000_000) return null;
    const value = Number(buf.readBigUInt64LE(off)); off += 8;
    const consumables_count = buf.readUInt8(off); off += 1;
    const non_consumables_count = buf.readUInt8(off); off += 1;
    const outputs_count = buf.readUInt8(off); off += 1;
    const total_count = buf.readUInt16LE(off); off += 2;
    if (total_count > 512) return null; // sanity guard

    const recipe_items: Array<any> = [];
    ...
    for (let i = 0; i < total_count; i++) {
      ...
      const amount = Number(buf.readBigUInt64LE(off)); off += 8;
      const mint = readPubkeyBase58(buf, off); off += 32;
      let key_index: number | undefined = undefined;
    ...
    if (recipe_items.length === 0 && total_count > 0) return null;
  ...
  try {
    ...
    if (found >= 0) off = found + RECIPE_DISCRIMINATOR.length;
    const version = safeReadUInt8(buf, off); off += 1;
    // require that next bytes look like pubkeys
    if (!looksLikePubkey(buf, off) || !looksLikePubkey(buf, off + 32) || !looksLikePubkey(buf, off + 64)) return null;
    const domain = readPubkeyBase58(buf, off); off += 32;
    const category = readPubkeyBase58(buf, off); off += 32;
    const creator = readPubkeyBase58(buf, off); off += 32;
    const duration = Number(safeReadBigUInt64LE(buf, off)); off += 8;
    const min_duration = Number(safeReadBigUInt64LE(buf, off)); off += 8;
    const namespace = buf.slice(off, off + 32); off += 32;
    const status = safeReadUInt8(buf, off); off += 1;
    const fee_amount = Number(safeReadBigUInt64LE(buf, off)); off += 8;
    let fee_recipient: string | null = null;
    if (off < buf.length) {
      const maybeFeeFlag = safeReadUInt8(buf, off); off += 1;
    ...
    while (off + 40 <= buf.length) {
      const amount = Number(safeReadBigUInt64LE(buf, off)); off += 8;
      const mint = readPubkeyBase58(buf, off); off += 32;
      recipe_items.push({ amount, mint });
    }
    if (recipe_items.length === 0) return null;
...
export function decodeCraftingProcess(data: Buffer | Uint8Array): any | null {
  const buf = Buffer.from(data);
  ...
  if (off + 165 > buf.length) return null;
  ...
  try {
    const version = buf.readUInt8(off); off += 1;
    const crafting_id = Number(buf.readBigUInt64LE(off)); off += 8;
    const authority = readPubkeyBase58(buf, off); off += 32;
    const recipe = readPubkeyBase58(buf, off); off += 32;
    const crafting_facility = readPubkeyBase58(buf, off); off += 32;
    const inputs_checksum = buf.slice(off, off + 16).toString('hex'); off += 16;
    const outputs_checksum = buf.slice(off, off + 16).toString('hex'); off += 16;
    // basic checksum sanity: ensure slices were available
    if (inputs_checksum.length !== 32 || outputs_checksum.length !== 32) return null;
    const quantity = Number(buf.readBigUInt64LE(off)); off += 8;
    const status = buf.readUInt8(off); off += 1;
    const start_time = Number(buf.readBigInt64LE(off)); off += 8;
    const end_time = Number(buf.readBigInt64LE(off)); off += 8;
    const deny_permissionless_claiming = buf.readUInt8(off); off += 1;
    const use_local_time = buf.readUInt8(off); off += 1;
...
export function decodeCraftableItem(data: Buffer | Uint8Array): any | null {
  const buf = Buffer.from(data);
  if (buf.length < CRAFTABLE_ITEM_DISCRIMINATOR.length) return null;
  let off = 0;
  ...
  try {
    ...
    const mint = readPubkeyBase58(buf, off); off += 32;
    const creator = readPubkeyBase58(buf, off); off += 32;
    const namespace = buf.slice(off, off + 32).toString('hex'); off += 32;
    const bump = buf.readUInt8(off); off += 1;
