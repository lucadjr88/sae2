...
export function normalizeRustDecode(parsed: any): any | null {
  ...
  if (!result.recipe && !result.process && !result.item && typeof data === 'object') {
    ...
    if ('recipe_items' in flat || 'fee_amount' in flat) result.recipe = { ...outer, data: { Recipe: flat } };
    else if ('crafting_id' in flat || 'start_time' in flat) result.process = { ...outer, data: { Process: flat } };
    else if ('mint' in flat || 'namespace' in flat) result.item = { ...outer, data: { Item: flat } };
  }
