...
export async function getWalletSageFeesDetailedStreaming(
  ...
  preloadedPayload?: { ownerAddress?: string; ownerTxs?: any[]; payerAddress?: string; payerTxs?: any[] }
): Promise<any> {
  ...
  let totalSigs = 0;
  // If a preloaded payload is provided by the caller (preloader), reuse it to avoid duplicate RPC fetches
  if (preloadedPayload && Array.isArray(preloadedPayload.payerTxs) && preloadedPayload.payerAddress === walletPubkey) {
    allTransactions = preloadedPayload.payerTxs;
    totalSigs = allTransactions.length;
    nlog(`[stream] Reusing preloaded payerTxs for ${walletPubkey.substring(0,8)} (${totalSigs} txs)`);
  } else if (preloadedPayload && Array.isArray(preloadedPayload.ownerTxs) && preloadedPayload.ownerAddress === walletPubkey) {
    // fallback: if ownerTxs were loaded for the same wallet, reuse them
    allTransactions = preloadedPayload.ownerTxs;
    totalSigs = allTransactions.length;
    nlog(`[stream] Reusing preloaded ownerTxs for ${walletPubkey.substring(0,8)} (${totalSigs} txs)`);
  } else {
  ...
  for (let i = 0; i < allTransactions.length; i += BATCH_SIZE) {
    ...
    for (const tx of batch) {
      ...
      const _sig = (tx as any).signature || (tx as any).txid || ((tx as any).transaction && (tx as any).transaction.signatures && (tx as any).transaction.signatures[0]) || 'unknown';
      try {
        const pidPreview = Array.isArray((tx as any).programIds) ? (tx as any).programIds.slice(0, 6).join(',') : 'none';
      } catch (e) {
        // ignore logging errors
      }
      processedTransactions.push(tx);
      totalFees24h += tx.fee;
      const _includesSage = Array.isArray((tx as any).programIds) && (tx as any).programIds.includes(SAGE_PROGRAM_ID);
      if (!_includesSage) {
        continue;
      }
      sageFees24h += tx.fee;
    ...
    // batch summary logging removed (was tagged [crafting-details])
