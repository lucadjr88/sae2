...
export async function getFleets(rpcEndpoint: string, rpcWebsocket: string, walletPath: string, profileId: string) {
  ...
  if (walletAuthority) {
    try {
      ...
      while (walletSignatures.length < maxToAnalyze) {
        ...
        try {
          ...
          if (err?.message?.includes('429')) {
            console.log('[wallet-scan] Rate limited (429), backoff delay:', currentDelay, 'ms');
            consecutiveErrors++;
        ...
        console.log(`[wallet-scan] Batch ${fetchBatchCount}: ${batch.length} sigs, total: ${walletSignatures.length}, delay: ${currentDelay}ms | RPC: ${healthyCount}/${metrics.length} healthy, ${totalProcessed} processed, ${avgLatency}ms avg latency`);
      ...
      console.log(`[tx-analysis] RPC pool status: ${preHealthy}/${preAnalysisMetrics.length} healthy endpoints`);
