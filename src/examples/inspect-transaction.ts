...
async function loadRpc() {
  try {
    ...
    if (rpcPool && rpcPool.length > 0) return rpcPool[0].url;
  } catch (e) {
    // ignore
  }
...
const MATERIAL_MINTS: { [mint: string]: string } = {
...
function short(s: string | undefined, len = 8) {
  if (!s) return '';
  return s.length > len ? s.substring(0, len) + '...' : s;
}

function detectMaterialFromParsed(parsed: any) {
  if (!parsed) return undefined;
  const fields = [] as string[];
  if (parsed.info) {
    Object.values(parsed.info).forEach(v => { if (typeof v === 'string') fields.push(v); });
  }
  for (const f of fields) {
    if (typeof f !== 'string') continue;
    if (/fuel/i.test(f)) return 'Fuel';
    if (/ore/i.test(f)) return 'Ore';
    if (/tool/i.test(f)) return 'Tool';
    if (/component/i.test(f)) return 'Component';
    if (/food/i.test(f)) return 'Food';
    if (/ammo/i.test(f)) return 'Ammo';
    if (MATERIAL_MINTS[f]) return MATERIAL_MINTS[f];
  }
  return undefined;
}
...
async function main() {
  ...
  const sigs = (tx as any)?.transaction?.signatures || (tx as any)?.signatures || [];
  console.log('signatures:', sigs);
  console.log('\n--- Message instructions ---');
  const msgIxs = (tx.transaction.message.instructions || []) as any[];
  msgIxs.forEach((ix: any, idx: number) => {
    ...
    if (ix.parsed) {
      ...
      if (mat) console.log('   -> detected material:', mat);
    } else {
  ...
  for (const blk of inner) {
    ...
    for (const iin of (blk.instructions || []) as any[]) {
      const parsed = (iin as any).parsed;
      ...
      if (parsed) {
        ...
        console.log('     raw data:', short((iin as any).data));
      }
    }
  }
  ...
  try {
    ...
    if (pre.length || post.length) {
      ...
      pre.forEach((p: any) => console.log(' ', p.mint, 'owner:', p.owner, 'uiTokenAmount:', p.uiTokenAmount));
      console.log('postTokenBalances:');
      post.forEach((p: any) => console.log(' ', p.mint, 'owner:', p.owner, 'uiTokenAmount:', p.uiTokenAmount));
    }
  } catch (e) {}
  ...
  for (const l of logs) {
    ...
    if (m) opCandidates.push(m[1] || m[0]);
    if (/craft/i.test(l)) opCandidates.push('craft');
    if (/process/i.test(l)) opCandidates.push('process');
    if (/mint/i.test(l)) opCandidates.push('mint');
  }
  ...
  const mintsFound = new Set<string>();
  for (const blk of inner) {
    for (const iin of (blk.instructions || []) as any[]) {
      const info = (iin as any)?.parsed?.info || {};
      const fields = [info.mint, info.destination, info.source, info.authority];
      for (const f of fields) if (typeof f === 'string') mintsFound.add(f);
    }
  }
  if (mintsFound.size) {
    console.log('Candidate mints found in inner instructions:');
    for (const m of mintsFound) console.log(' ', m, '->', MATERIAL_MINTS[m] || 'unknown');
  }
  ...
  async function tryMetadata(mint: string) {
    try {
      ...
      if (!acct || !acct.data) return undefined;
