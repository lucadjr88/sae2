...
export async function getWalletSageFeesDetailedStreaming(rpcEndpoint, rpcWebsocket, walletPubkey, fleetAccounts, fleetAccountNames = {}, fleetRentalStatus = {}, hours = 24, sendUpdate, saveProgress, cachedData, lastProcessedSignature, preloaded = undefined) {
    ...
    // If preloaded transactions were provided (from preloader), reuse them and avoid a second wallet-scan
    let allTransactions = null;
    let totalSigs = 0;
    console.log(`[crafting-details] DEBUG: preloaded type=${typeof preloaded}, value=${preloaded ? JSON.stringify({...preloaded, ownerTxs: preloaded.ownerTxs ? `[${preloaded.ownerTxs.length}]` : undefined, payerTxs: preloaded.payerTxs ? `[${preloaded.payerTxs.length}]` : undefined}) : 'null'}`);
    try {
        if (preloaded && (Array.isArray(preloaded.ownerTxs) || Array.isArray(preloaded.payerTxs))) {
            console.log(`[crafting-details] Using preloaded txs: owner=${Array.isArray(preloaded.ownerTxs) ? preloaded.ownerTxs.length : 0}, payer=${Array.isArray(preloaded.payerTxs) ? preloaded.payerTxs.length : 0}`);
            // Merge ownerTxs and payerTxs (if present) and deduplicate by signature
            const combined = [];
            const seen = new Set();
            const pushTx = (t) => {
                if (!t) return;
                // Try to find signature fields but tolerate objects without signature
                const sig = t?.signature || t?.txid || (t.transaction && t.transaction.signatures && t.transaction.signatures[0]);
                if (sig) {
                    if (seen.has(sig)) {
                        //console.log(`[crafting-details] Skipping duplicate sig: ${sig.substring(0,8)}`);
                        return;
                    }
                    seen.add(sig);
                }
            ...
            if (Array.isArray(preloaded.ownerTxs)) {
                console.log(`[crafting-details] Processing ${preloaded.ownerTxs.length} owner txs`);
                for (const t of preloaded.ownerTxs) {
                    try {
                        if (!Array.isArray(t.programIds)) t.programIds = [];
                        if (Array.isArray(t.accountKeys) && t.accountKeys.includes(SAGE_PROGRAM_ID) && !t.programIds.includes(SAGE_PROGRAM_ID)) {
                            t.programIds.push(SAGE_PROGRAM_ID);
                        }
                        if (!Array.isArray(t.instructions) && Array.isArray(t.instructions) === false) t.instructions = t.instructions || [];
                    }
                    catch (e) {
                        // tolerate malformed preloaded tx shapes
                    }
                    pushTx(t);
                }
            }
            if (Array.isArray(preloaded.payerTxs)) {
                console.log(`[crafting-details] Processing ${preloaded.payerTxs.length} payer txs`);
                for (const t of preloaded.payerTxs) {
                    try {
                        if (!Array.isArray(t.programIds)) t.programIds = [];
                        if (Array.isArray(t.accountKeys) && t.accountKeys.includes(SAGE_PROGRAM_ID) && !t.programIds.includes(SAGE_PROGRAM_ID)) {
                            t.programIds.push(SAGE_PROGRAM_ID);
            ...
            // Use combined even if signature detection failed for some items — prefer preloaded to avoid extra RPC
            if (combined.length > 0) {
                console.log(`[crafting-details] Combined preloaded txs: ${combined.length} after dedup, ${seen.size} unique sigs`);
                allTransactions = combined;
                // If we have deduplicated signatures count, use it, otherwise fall back to array length
                totalSigs = seen.size > 0 ? seen.size : combined.length;
            } else {
                console.warn(`[crafting-details] Preloaded txs present but combined resulted in 0 txs! ownerTxs=${preloaded.ownerTxs?.length}, payerTxs=${preloaded.payerTxs?.length}`);
            }
        }
    } catch (e) {
        // ignore preloaded shape errors and fallback to fetching
        console.error(`[crafting-details] Error processing preloaded txs:`, e);
    }
    ...
    for (let i = 0; i < allTransactions.length; i += BATCH_SIZE) {
        ...
        for (const tx of batch) {
            ...
            if (isCrafting) {
                ...
                // Prefer extracting material actions from the already-parsed `tx` to avoid extra RPC calls
                let materialActions = [];
                try {
                    // If parsed transaction contains meta and innerInstructions, extract locally
                    if (tx && tx.meta && Array.isArray(tx.meta.innerInstructions) && tx.meta.innerInstructions.length > 0) {
                        // Walk inner instructions to find parsed token transfers/burns
                        for (const blk of tx.meta.innerInstructions) {
                            if (!blk || !Array.isArray(blk.instructions)) continue;
                            for (const iin of blk.instructions) {
                                const parsed = iin.parsed || iin;
                                if (!parsed) continue;
                                if (parsed.info && parsed.info.mint) {
                                    const tokenId = parsed.info.mint;
                                    const amount = Number(parsed.info.amount || 0);
                                    let action = 'other';
                                    const logStr = (tx.meta.logMessages || []).join('\n');
                                    const decoded = decodeSageInstruction(logStr) || {};
                                    ...
                                    materialActions.push({
                                        txSignature: tx.signature || tx.txid || tx.transaction?.signatures?.[0] || 'unknown',
                                        instruction: decoded.name || decoded.type || 'unknown',
                                        action,
                                        tokenId,
                        ...
                        // Fallback: try light extraction from logMessages only
                        const logs = tx.meta?.logMessages || [];
                        for (const log of logs) {
                            const decoded = decodeSageInstruction(log);
                            if (decoded) {
                                materialActions.push({
                                    txSignature: tx.signature || tx.txid || 'unknown',
                                    instruction: decoded.name || decoded.type || 'unknown',
                                    action: /burn/i.test(decoded.name || '') ? 'burn' : (/deposit/i.test(decoded.name || '') ? 'deposit' : (/withdraw/i.test(decoded.name || '') ? 'withdraw' : 'other')),
                                    tokenId: decoded.mint || '',
                                    materialName: decoded.material,
                                    amount: 0
                                });
                            }
                    ...
                    console.warn('[wallet-sage-fees-streaming] local material extraction failed:', e);
                }
            ...
            const _sig = tx.signature || tx.txid || (tx.transaction && tx.transaction.signatures && tx.transaction.signatures[0]) || 'unknown';
            try {
                const pidPreview = Array.isArray(tx.programIds) ? tx.programIds.slice(0,6).join(',') : 'none';
                console.log(`[crafting-details] TX CHECK sig=${(_sig && _sig.substring) ? _sig.substring(0,8) : _sig} isCrafting=${!!isCrafting} programIds=${pidPreview} meta=${tx.meta? 'yes':'no'}`);
            }
            catch (e) {
                // ignore logging errors
            }
            processedTransactions.push(tx);
            console.log(`[crafting-details] TX PUSHED sig=${(_sig && _sig.substring) ? _sig.substring(0,8) : _sig} totalProcessed=${processedTransactions.length}`);
            totalFees24h += tx.fee;
            const _includesSage = Array.isArray(tx.programIds) && tx.programIds.includes(SAGE_PROGRAM_ID);
            if (!_includesSage) {
                console.log(`[crafting-details] TX SKIP-NOT-SAGE sig=${(_sig && _sig.substring) ? _sig.substring(0,8) : _sig} programIds=${Array.isArray(tx.programIds)?tx.programIds.slice(0,6).join(','):'none'}`);
                continue;
            }
            console.log(`[crafting-details] TX IS-SAGE sig=${(_sig && _sig.substring) ? _sig.substring(0,8) : _sig}`);
            sageFees24h += tx.fee;
        }
