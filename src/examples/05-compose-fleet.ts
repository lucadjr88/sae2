...
export async function getShipsForFleet(rpcEndpoint: string, rpcWebsocket: string, walletPath: string, profileId: string, x: number, y: number) {
  ...
  for (const ship of ships) {
    if (ship.type !== 'ok') continue;
    const data = ship.data.data as any;
