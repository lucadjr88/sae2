...
console.log = (...args: any[]) => { __orig_console_log(...args); appendLogLine(args.map(a => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')); };
console.error = (...args: any[]) => { __orig_console_error(...args); appendLogLine('[ERROR] ' + args.map(a => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')); };
console.warn = (...args: any[]) => { __orig_console_warn(...args); appendLogLine('[WARN] ' + args.map(a => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')); };
...
async function collectCandidatePubkeys(): Promise<string[]> {
...
function deriveSagePdas(ids: string[]): string[] {
  const seedsToTry = [
    'recipe',
    'recipe_v1',
    'process',
  ...
  try {
    ...
    for (const id of ids) {
      let pk: PublicKey;
      try { pk = new PublicKey(id); } catch (e) { continue; }
      ...
      try { const [p] = PublicKey.findProgramAddressSync([pk.toBuffer()], prog); out.add(p.toString()); } catch (e) {}
    }
  } catch (e) {
...
async function batchFetch(pubkeys: string[], batchSize = 12) {
  const all: Array<{ pubkey: string; data: Buffer | null; err?: any }> = [];
...
async function fetchAndCacheAccounts(pubkeys: string[]) {
  ...
  const conn: Connection = picked.connection as Connection;
  const idx = picked.index;
  const results: Array<{ pubkey: string; data: Buffer | null; err?: any }> = [];
  ...
  try {
    ...
    for (let i = 0; i < pubkeys.length; i++) {
      ...
      const b = Buffer.from(info.data as Buffer);
      const filePath = path.join(CACHE_DIR, `${pubkeys[i]}.base64`);
      try { await writeFile(filePath, b.toString('base64'), 'utf8'); } catch (e) {}
...
function bufferFromFileContent(filePath: string): Buffer | null {
  try {
...
async function run() {
  ...
  if (existing.length === 0 && !SKIP_SEED) {
    ...
    for (const r of fetched) if (r.data) testedAccounts.push(r.pubkey);
  } else {
    ...
    if (LIMIT && LIMIT > 0) console.log('[test-decoders] Applying limit:', LIMIT);
  ...
  for (const pk of testedAccounts) {
    ...
    if (r) summary.recipe++;
    if (p) summary.process++;
    if (it) summary.item++;

    console.log('---');
    console.log(pk);
    console.log('rust:', rust ? 'OK' : 'null');
    if (r) console.log('Recipe decoded:', JSON.stringify(r, null, 2));
    if (p) console.log('Process decoded:', JSON.stringify(p, null, 2));
    if (it) console.log('CraftableItem decoded:', JSON.stringify(it, null, 2));
    if (!r && !p && !it && !rust) console.log('No decoder produced output for this account.');
  }
