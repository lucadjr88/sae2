...
import { getGlobalWalletTransactionPreloader } from './utils/wallet-transaction-preloader.js';
...
// Debug: Inspect raw transaction cache for a wallet
app.get('/api/debug/tx-cache/:wallet', async (req, res) => {
  ...
  try {
    ...
    const cached = await getCacheWithTimestamp('transaction-cache', cacheKey);
    if (!cached) {
      return res.status(404).json({ error: 'No cache found for this wallet and period' });
    }
    return res.json({
      wallet,
      hours: Number(hours),
      savedAt: cached.savedAt,
      txCount: Array.isArray(cached.data.txs) ? cached.data.txs.length : 0,
      txs: cached.data.txs || [],
    });
  } catch (err: any) {
    const errorMsg = (err && typeof err === 'object' && 'message' in err) ? (err as any).message : String(err);
    console.error('/api/debug/tx-cache error:', errorMsg);
...
app.post('/api/wallet-sage-fees-stream', async (req, res) => {
  ...
  res.setHeader('Connection', 'keep-alive');
  res.flushHeaders();
  // RPC snapshot for per-request counters (initialized before preloader)
  let rpcSnapshotStart: any = null;

  const sendUpdate = (data: any) => {
    try {
      // Compute RPC delta since request start if available
      if (rpcSnapshotStart) {
        try {
          const now = getRpcMetrics();
          const sum = (arr: any[]) => arr.reduce((acc, v) => ({ successes: acc.successes + (v.successes || 0), failures: acc.failures + (v.failures || 0) }), { successes: 0, failures: 0 });
          const nowSum = sum(now);
          const startSum = sum(rpcSnapshotStart);
          const deltaSuccess = nowSum.successes - startSum.successes;
          const deltaFail = nowSum.failures - startSum.failures;
          const deltaTotal = deltaSuccess + deltaFail;
          data.rpcCalls = deltaTotal;
          data.rpcSuccesses = deltaSuccess;
          data.rpcFailures = deltaFail;
        } catch (e) {
          // ignore metric attach errors
        }
      }
  ...
  try {
    // If walletOwner not provided, attempt to derive it from profileId via getFleets
    let walletOwner: string | null = (req.body && req.body.walletOwner) ? req.body.walletOwner : null;
    if (!walletOwner && req.body && req.body.profileId) {
      const profileId = req.body.profileId;
      console.log(`[stream] walletOwner missing - attempting to derive from profileId=${profileId.substring(0,8)}...`);
      // First try: fetch player profile account (single account RPC) to extract authority wallet
      try {
        const profileRes = await getPlayerProfile(RPC_ENDPOINT, RPC_WEBSOCKET, WALLET_PATH, profileId);
        if (profileRes && profileRes.authority) {
          walletOwner = profileRes.authority;
          console.log(`[stream] Derived walletOwner via getPlayerProfile: ${walletOwner.substring(0,8)}...`);
        }
      } catch (e) {
        console.warn('[stream] getPlayerProfile failed to derive walletOwner (will fallback to getFleets):', (e as any)?.message || e);
      }

      // Fallback: use getFleets (more expensive) with retries if profile lookup didn't yield an owner
      if (!walletOwner) {
        const maxAttempts = 3;
        let attempt = 0;
        while (attempt < maxAttempts && !walletOwner) {
          attempt++;
          try {
            const fleetsRes = await getFleets(RPC_ENDPOINT, RPC_WEBSOCKET, WALLET_PATH, profileId);
            if (fleetsRes && (fleetsRes as any).walletOwner) {
              walletOwner = (fleetsRes as any).walletOwner;
              console.log(`[stream] Derived walletOwner via getFleets: ${walletOwner ? walletOwner.substring(0,8) : 'unknown'}... (attempt ${attempt})`);
              break;
            } else {
              console.log(`[stream] getFleets did not return walletOwner (attempt ${attempt})`);
            }
          } catch (e: any) {
            const msg = e?.message || String(e);
            const is429 = msg.includes('429') || msg.includes('Too many request') || e?.code === 15;
            const backoff = is429 ? 500 * Math.pow(2, attempt - 1) : 500 * attempt;
            if (is429 && attempt < maxAttempts) {
              console.warn(`[stream] getFleets rate-limited (attempt ${attempt}/${maxAttempts}), retrying in ${backoff}ms`);
              await new Promise(resolve => setTimeout(resolve, backoff));
              continue;
            } else {
              console.warn('[stream] getFleets failed to derive walletOwner:', msg);
              break;
            }
          }
        }
        if (!walletOwner) {
          console.log('[stream] Failed to derive walletOwner after retries; proceeding without owner (will not mistakenly treat fee payer as owner)');
        }
      }
    }

    // Preload transactions for owner + fee payer to avoid duplicate fetches
    // Prepare preloaded payload container (may remain null if preloader fails)
    let _preloadedPayload: any = null;
    try {
      // Initialize per-request RPC snapshot now so preloader RPCs are counted
      rpcSnapshotStart = getRpcMetrics();

      const preloader = getGlobalWalletTransactionPreloader(RPC_ENDPOINT, RPC_WEBSOCKET, globalPoolConnection);
      const ownerForPreload = walletOwner || walletPubkey;
      console.log(`[stream] Preloading transactions for owner=${ownerForPreload.substring(0,8)}..., payer=${walletPubkey.substring(0,8)}...`);
      const preloadRes = await preloader.preloadForProfile(ownerForPreload, walletPubkey, { hours: hours || 24, refresh });
      const stats = preloader.getStats();
      console.log('[stream] Preloader stats:', stats);
      // Debug: summarize preloaded payload before passing downstream
      console.log('[stream] _preloadedPayload summary:', {
        ownerAddress: ownerForPreload,
        ownerTxsCount: Array.isArray(preloadRes?.ownerTransactions) ? preloadRes.ownerTransactions.length : typeof preloadRes?.ownerTransactions,
        payerAddress: walletPubkey,
        payerTxsCount: Array.isArray(preloadRes?.payerTransactions) ? preloadRes.payerTransactions.length : typeof preloadRes?.payerTransactions,
      });
      // Prepare preloaded payload to pass downstream and avoid duplicate wallet-scan
      _preloadedPayload = {
        ownerAddress: ownerForPreload,
        ownerTxs: preloadRes.ownerTransactions,
        payerAddress: walletPubkey,
        payerTxs: preloadRes.payerTransactions
      };
    } catch (preErr) {
      console.warn('[stream] Preloader failed (continuing):', (preErr as any)?.message || preErr);
      _preloadedPayload = null;
      // Even if preloader failed, initialize rpcSnapshotStart to begin counting RPCs for the rest of the request
      if (!rpcSnapshotStart) rpcSnapshotStart = getRpcMetrics();
    }

    const { getWalletSageFeesDetailedStreaming } = await import('./examples/wallet-sage-fees-streaming.js');
    
    const finalResult = await (getWalletSageFeesDetailedStreaming as any)(
      RPC_ENDPOINT,
      RPC_WEBSOCKET,
      walletPubkey,
      fleetAccounts || [],
    ...
    try {
      // Log summary counts: unique signatures, unique txs after dedup, total RPC calls
      const uniqueSignatures = finalResult && finalResult.totalSignaturesFetched ? finalResult.totalSignaturesFetched : 0;
      const uniqueTxsAfterDedup = finalResult && finalResult.transactions ? finalResult.transactions.length : (finalResult && finalResult.transactionCount24h) || 0;
      // Compute per-request RPC delta if snapshot exists
      let totalRpcCalls = 0;
      let rpcSuccesses = 0;
      let rpcFailures = 0;
      try {
        const rpcMetricsArr = getRpcMetrics();
        if (rpcSnapshotStart) {
          const sum = (arr: any[]) => arr.reduce((acc, v) => ({ successes: acc.successes + (v.successes || 0), failures: acc.failures + (v.failures || 0) }), { successes: 0, failures: 0 });
          const nowSum = sum(rpcMetricsArr);
          const startSum = sum(rpcSnapshotStart);
          rpcSuccesses = nowSum.successes - startSum.successes;
          rpcFailures = nowSum.failures - startSum.failures;
          totalRpcCalls = rpcSuccesses + rpcFailures;
        } else {
          totalRpcCalls = rpcMetricsArr.reduce((s: number, m: any) => s + (m.successes || 0) + (m.failures || 0), 0);
        }
      } catch (e) {
        // ignore metric errors
      }
      nlog(`[stream] Summary: uniqueSignatures=${uniqueSignatures}, uniqueTxsAfterDedup=${uniqueTxsAfterDedup}, totalRpcCalls=${totalRpcCalls}, rpcSuccesses=${rpcSuccesses}, rpcFailures=${rpcFailures}`);
    } catch (e) {
...
// Check wallet transaction cache status
app.post('/api/cache/wallet-status', async (req, res) => {
  const { walletPubkey, hours } = req.body || {};
  if (!walletPubkey) return res.status(400).json({ error: 'walletPubkey required' });
  const h = hours || 24;
  const cacheKey = `tx-cache-${walletPubkey}-${h}h`;
  try {
    const cached = await getCacheWithTimestamp<any>('transaction-cache', cacheKey);
    if (!cached) return res.json({ cached: false, cacheKey });
    const ageMs = Date.now() - cached.savedAt;
    const ageMin = +(ageMs / 60000).toFixed(1);
    const txCount = Array.isArray(cached.data?.txs) ? cached.data.txs.length : (cached.data?.txs?.length || 0);
    return res.json({ cached: true, cacheKey, savedAt: cached.savedAt, ageMs, ageMin, txCount });
  } catch (e: any) {
    console.error('/api/cache/wallet-status error:', e?.message || e);
    return res.status(500).json({ error: e?.message || String(e) });
  }
});

// Force preload and save wallet transactions to cache (uses preloader)
app.post('/api/cache/wallet-preload', async (req, res) => {
  const { walletPubkey, profileId, walletOwner, hours } = req.body || {};
  if (!walletPubkey && !profileId && !walletOwner) return res.status(400).json({ error: 'walletPubkey or profileId or walletOwner required' });
  const h = hours || 24;
  try {
    // Derive owner if profileId provided and walletOwner not
    let owner = walletOwner || null;
    if (!owner && profileId) {
      try {
        const p = await getPlayerProfile(RPC_ENDPOINT, RPC_WEBSOCKET, WALLET_PATH, profileId);
        if (p && p.authority) owner = p.authority;
      } catch {}
      if (!owner) {
        try {
          const fr = await getFleets(RPC_ENDPOINT, RPC_WEBSOCKET, WALLET_PATH, profileId);
          if (fr && (fr as any).walletOwner) owner = (fr as any).walletOwner;
        } catch {}
      }
    }

    const ownerForPreload = owner || walletPubkey;
    const preloader = getGlobalWalletTransactionPreloader(RPC_ENDPOINT, RPC_WEBSOCKET, globalPoolConnection);
    const preloadRes = await preloader.preloadForProfile(ownerForPreload, walletPubkey || ownerForPreload, { hours: h, refresh: true });
    // The TransactionCache should have saved the parsed txs to disk during preload via getAccountTransactions flow
    const cacheKey = `tx-cache-${(walletPubkey || ownerForPreload)}-${h}h`;
    const diskCached = await getCacheWithTimestamp<any>('transaction-cache', cacheKey);
    const txCount = Array.isArray(preloadRes.ownerTransactions) ? preloadRes.ownerTransactions.length : 0;
    return res.json({ success: true, owner: ownerForPreload, ownerTxCount: preloadRes.ownerTransactions?.length || 0, payerTxCount: preloadRes.payerTransactions?.length || 0, cacheKey, diskCached });
  } catch (e: any) {
    console.error('/api/cache/wallet-preload error:', e?.message || e);
    return res.status(500).json({ error: e?.message || String(e) });
  }
});
...
app.post('/api/diagnostics/fleet-map', async (req, res) => {
  ...
  try {
    const fleetsRes: any = await getFleets(RPC_ENDPOINT, RPC_WEBSOCKET, WALLET_PATH, profileId);
    const { fleets, walletAuthority, ownerByKey } = fleetsRes || {};
    ...
    res.json({ success: true, walletAuthority, rows, map, ownerByKey });
  } catch (err: any) {
...
// Pre-request endpoint: preload owner + payer transactions and return merged counts
app.post('/api/prerequest/preload-wallet', async (req, res) => {
  const { walletPubkey, profileId, walletOwner: providedOwner } = req.body || {};
  const hours = (req.body && req.body.hours) || 24;
  const refresh = (req.query.refresh === 'true') || (req.body && req.body.refresh === true);

  if (!walletPubkey && !profileId && !providedOwner) {
    return res.status(400).json({ error: 'walletPubkey or profileId or walletOwner required' });
  }

  try {
    // Attempt to derive walletOwner if not provided
    let walletOwner: string | null = providedOwner || null;
    if (!walletOwner && profileId) {
      try {
        const profileRes = await getPlayerProfile(RPC_ENDPOINT, RPC_WEBSOCKET, WALLET_PATH, profileId);
        if (profileRes && profileRes.authority) walletOwner = profileRes.authority;
      } catch (e) {
        // ignore, will fallback to getFleets
      }
    ...
    const ownerForPreload = walletOwner || walletPubkey;
    const preloader = getGlobalWalletTransactionPreloader(RPC_ENDPOINT, RPC_WEBSOCKET, globalPoolConnection);
    const preloadRes = await preloader.preloadForProfile(ownerForPreload, walletPubkey || ownerForPreload, { hours, refresh });

    const ownerTxs = preloadRes.ownerTransactions || [];
    const payerTxs = preloadRes.payerTransactions || [];

    // Merge and deduplicate by signature
    const seen = new Set();
    const mergedSignatures = [];
    const normalizeSig = (t: any) => {
      if (!t) return null;
      return t.signature || t.txid || (t.transaction && Array.isArray(t.transaction.signatures) && t.transaction.signatures[0]) || null;
    };
    for (const t of ownerTxs) {
      const s = normalizeSig(t);
      if (!s) continue;
      if (!seen.has(s)) {
        seen.add(s);
        mergedSignatures.push(s);
      }
    }
    for (const t of payerTxs) {
      const s = normalizeSig(t);
      if (!s) continue;
      if (!seen.has(s)) {
        seen.add(s);
    ...
    // Return lightweight summary (avoid returning full tx arrays)
    return res.json({
      success: true,
      ownerAddress: ownerForPreload,
      ownerTxCount: ownerTxs.length,
      payerTxCount: payerTxs.length,
      mergedTxCount: mergedSignatures.length,
      sampleSignatures: mergedSignatures.slice(0, 20)
    });
  } catch (err: any) {
    console.error('/api/prerequest/preload-wallet error:', err && err.message ? err.message : err);
